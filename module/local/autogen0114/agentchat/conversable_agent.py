...
class ConversableAgent(Agent):
    ...
    def generate_oai_reply(
        ...
        _replay = oai.ChatCompletion.extract_text_or_function_call(response)[0]
        # print(f"【context】", messages[-1].pop("context", None))
        print(f"【messages】", self._oai_system_message + messages)
        print(f"【last message】", messages[-1])
        if 'execution succeeded' in messages[-1]['content']:
